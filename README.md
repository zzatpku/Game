...
- 第三阶段重点游客会随机顺时针或逆时针巡逻，但大部分时间不会移动：先停留 10 秒，再移动 20 秒循环。跟随游客主要在重点游客移动的 20 秒里出现，每个移动窗口约 2 人；这些游客会持续乱丢垃圾。重点游客和鸭子都有血条。`Space` 仍用于捡垃圾、把垃圾投进垃圾桶以及空嘴时鸭叫；鸭子也可以按鼠标左键投掷已叼起的垃圾，命中重点游客会降低重点游客血量并让湖面更干净，落空则垃圾重新掉回湖里。游客投掷命中鸭子会降低鸭子血量。
