...
[![直接打 Boss](https://img.shields.io/badge/直接打%20Boss-王乱丢关卡-b33327?style=for-the-badge)](https://zzatpku.github.io/Game/?debugStage=3)
[![从头试玩](https://img.shields.io/badge/从头试玩-未名湖守护鸭-2d8064?style=for-the-badge)](https://zzatpku.github.io/Game/)

这是根据 `game.txt` 做的迷你可玩演示版。玩家控制一只生活在未名湖的鸭子，处理游客投放到湖里的垃圾，通过鸣叫或告示牌提醒游客，并在第三阶段用垃圾投掷击退污染煽动者王乱丢。
...
Debug 直进第三阶段时，鸭子的携带上限会设为 3，等同于前两关都选择了携带升级。
...
- 鸣叫会产生声波，范围内的游客会被提醒。第一阶段完成后会解锁“护湖”告示牌，提醒范围和效果更强。
- 阶段之间会暂停并选择一次升级：一次携带更多垃圾、体力上限提高，或增强鸣叫范围。
- 第三阶段按鼠标左键投掷已叼起的垃圾攻击 Boss，按 `1` / `2` / `3` 使用道具槽。
...
- 第三阶段重点游客会随机顺时针或逆时针巡逻，但大部分时间不会移动：先停留 10 秒，再移动 20 秒循环。Boss 每 30 秒周期只会在移动阶段召来 1 名跟随游客；跟随游客的伤害、投掷间隔、瞄准和污染热点概率接近第一阶段普通游客。重点游客和鸭子都有血条，Boss 需要约 40 次垃圾投掷命中才能赶走；清澈度到 0% 后，鸭子每秒损失 2 点血。湖心岛每 30 秒刷新 1 个道具，新道具会覆盖旧道具；鸭子最多存 3 个道具，按 `1` / `2` / `3` 使用。道具包括 30 秒净水捡拾、30 秒弹带容量 +3、10 秒定住 Boss、30 秒无限体力。`Space` 仍用于捡垃圾、把垃圾投进垃圾桶以及空嘴时鸭叫；鸭子也可以按鼠标左键投掷已叼起的垃圾，命中重点游客会降低重点游客血量并让湖面更干净，落空则垃圾重新掉回湖里。游客投掷命中鸭子会降低鸭子血量。
- 前两阶段清澈度归零会失败；第三阶段鸭子血量归零会失败；赶走重点游客后显示胜利，并进入“第二阶段（尚未完成）”占位状态。
